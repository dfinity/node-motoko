...
function parseGithubPackage(package, name) {
    if (!package) {
        return;
    }
...
module.exports = {
    async loadPackages(mapping) {
        for (const [name, package] of Object.entries(mapping)) {
            const mo = require('.');
            const info = parseGithubPackage(package, name);
